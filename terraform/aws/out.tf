...
output "context_hub" {
  depends_on = [module.cluster-hub]
  value = <<EOF
  nats context save hub --user ${var.sys_user} --password ${var.sys_psw} --server "%{~ for i, v in module.cluster-hub.private_ip ~}nats://${v}:4222, %{~ endfor ~}"
EOF
}
...
output "context_spoke_1" {
  depends_on = [module.cluster-spoke-1]
  value = <<EOF
  nats context save spoke-1 --user ${var.sys_user} --password ${var.sys_psw} --server "%{~ for i, v in module.cluster-spoke-1.private_ip ~}nats://${v}:4222, %{~ endfor ~}"
EOF
}